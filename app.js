...
app.post('/predict', (req, res) => {
  const symptoms = req.body.symptoms;  // Get symptom data from frontend

  // Convert symptom array to space-separated string for command-line arguments
  ...
  pythonProcess.on('close', (code) => {
    if (code === 0) {
      try {
        const result = JSON.parse(output);  // Parse the JSON output from the Python script
        res.json(result);
      } catch (err) {
